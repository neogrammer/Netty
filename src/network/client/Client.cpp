...
//    GameStateManager gsm;
//    gsm.registerState("title", std::make_unique<TitleState>(&window, &gsm));
//    gsm.registerState("play", std::make_unique<PlayState>(&window, &entities, &udp_socket, &tcp_socket, server_ip, server_udp_port, player_id, entityAnimSets));
//    gsm.switchTo("title");   // or directly to "play" if no menu
//
//
//    while (window.isOpen()) {
//        // ---- Events ----
